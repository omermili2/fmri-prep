...
def deep_dive_suds():
    ...
    subset = df_clin[df_clin['group'] == 'B'].dropna(subset=['suds_change', 'caps_improvement'])
    
    print("Group B (Low-Dose Ketamine) Subject Data:")
    for _, row in subset.iterrows():
        print(f"Sub: {row['record_id']}, PE2_SUDS: {row['PE2_SUDS_mean']:.1f}, PE5_SUDS: {row['PE5_SUDS_mean']:.1f}, SUDS_Change: {row['suds_change']:.1f}, CAPS_Improv: {row['caps_improvement']}")
