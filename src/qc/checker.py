...
class BIDSQualityChecker:
    ...
    def _check_missing_scans(
        ...
        # 1. Check session-level anat/ (case-insensitive glob)
        if anat_dir.is_dir():
            if any("t1w" in f.name.lower() and f.name.endswith(".nii.gz") for f in anat_dir.iterdir()):
                has_t1w = True
        ...
        if not has_t1w:
            ...
            if sub_anat.is_dir():
                if any("t1w" in f.name.lower() and f.name.endswith(".nii.gz") for f in sub_anat.iterdir()):
                    has_t1w = True

        # Check for raw or processed BOLD
        has_bold = False
        if func_dir.is_dir():
            if any("bold" in f.name.lower() and f.name.endswith(".nii.gz") for f in func_dir.iterdir()):
                has_bold = True
